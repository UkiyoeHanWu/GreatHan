...
for epoch in range(EPOCH):
    for step, (b_x, b_y) in enumerate(train_loader):   # gives batch data, normalize x when iterate train_loader
        ...
        output = cnn(b_x.cuda())[0].cuda()               # cnn output
        loss = loss_func(output, b_y.cuda()).cuda()   # cross entropy loss
...
print(torch.max(test_output, 1)[1].data.cpu().numpy(), 'prediction number')
